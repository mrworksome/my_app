...
def load(*args, **kwargs):
    ...
    if created:
        user = VkUsers.objects.filter(username=kwargs.get('response').get('user_id')).first()
    ...
    if response.status_code == 200:
        ...
        for item in response.get("response").get("items"):
            ...
            friend = VkUsers.objects.filter(username=username).first()
            if not friend:
                VkUsers.objects.create(username=username, first_name=first_name, last_name=last_name)
                friend = VkUsers.objects.filter(username=username).first()
